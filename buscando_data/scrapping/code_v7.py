...
def wait_for_load_indicator_to_disappear(page: Page):
    print("Esperando a que la página termine de cargar (que 'shiny-busy' desaparezca del HTML)...")
    loading_indicator = page.locator("html.shiny-busy")
...
def run(playwright):
    ...
    try:
        # Usamos wait_for(visible) aquí porque es la carga INICIAL de la tabla. Es el mejor método para este caso.
        ...
        # --- SELECCIÓN DE PROVINCIA CON BUCLE DE REINTENTO ---
        ...
        for attempt in range(max_retries):
            ...
            # --- TUS PRINTS DE DEPURACIÓN AÑADIDOS ---
        ...
        print("Procesando, esperando contenido...")
        print("Paso 1: Esperando a que la página comience a cargar (aparezca 'html.shiny-busy')...")
        loading_indicator = page.locator("html.shiny-busy")
        loading_indicator.wait_for(state='visible', timeout=15000)
    ...
    print("\n¡Flujo de selección y procesamiento completado! El navegador se cerrará en 10 segundos.")
    time.sleep(10)
    browser.close()
