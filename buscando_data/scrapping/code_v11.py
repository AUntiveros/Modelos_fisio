...
def run(playwright):
    ...
    try:
        ...
        # ... [Toda la lógica de selección hasta Provincia es perfecta y se mantiene igual]
        ...
        # ... [Lógica de selección de Distrito es perfecta y se mantiene igual]
        max_retries_dist = 3
        success_dist = False
        ...
        print("Paso 1: Esperando a que la página comience a cargar (aparezca 'html.shiny-busy')...")
        page.locator("html.shiny-busy").wait_for(state='visible', timeout=15000)
        print("¡La página ha empezado a cargar!")
        print("Paso 2: Esperando a que la página termine de cargar...")
        page.locator("html.shiny-busy").wait_for(state='hidden', timeout=60000)
        print("Contenido actualizado con el filtro de distrito.")
        
        print("\nPreparando para la tercera descarga (con distrito)...")
        filename_3 = (f"{selected_year}-{selected_disease}-{selected_department.replace(' ', '-')}"
                      f"-{selected_province.replace(' ', '-')}-{selected_district.replace(' ', '-')}.xlsx")
        
        with page.expect_download() as download_info_3:
            print("Haciendo clic en el botón de descarga principal...")
            page.locator("#ten-tbl05b-download").click()
            
        download_3 = download_info_3.value
        download_3.save_as(os.path.join(download_folder, filename_3))
        print(f"¡Descarga 3 completa! Archivo guardado como: {filename_3}")
