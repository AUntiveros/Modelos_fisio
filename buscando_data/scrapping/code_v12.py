...
def run(playwright):
    ...
    try:
        ...
        # --- SELECCIONES INICIALES (HASTA PROVINCIA) ---
        ...
        for attempt in range(max_retries_prov):
            ...
            try:
                ...
                pass
        if not success_prov:
            raise Exception("No se pudieron cargar las opciones de Provincia. El script se detendrá.")
        
        
        print("\nHaciendo clic en el botón 'Procesar' (primera vez, sin distrito)...")
        ...
        print("\nPreparando para la primera descarga (sin distrito)...")
        ...
        print("\nPreparando para la segunda descarga (sin distrito)...")
        ...
        # --- PREPARACIÓN PARA EL BUCLE DE DISTRITOS ---
        print("\nObteniendo la lista de todos los distritos disponibles...")
        ...
        # --- BUCLE PRINCIPAL Y ROBUSTO PARA CADA DISTRITO ---
        for district_to_process in valid_districts:
            print(f"\n--- INICIANDO PROCESO PARA EL DISTRITO: {district_to_process} ---")
            
            # --- LÓGICA DE REINTENTO PARA LA SELECCIÓN DEL DISTRITO ACTUAL ---
            max_retries_dist = 3
            success_dist = False
            for attempt in range(max_retries_dist):
                print(f"Intento {attempt + 1}/{max_retries_dist} para seleccionar '{district_to_process}'...")
                page.locator("#ten-fdist-filter-selectized").click()
                visible_dist_options = page.locator(".selectize-dropdown-content:visible .option")
                ...
                try:
                    ...
                    if visible_dist_options.count() > 1:
                        print("¡Lista de distritos poblada con éxito!")
                        page.locator(".selectize-dropdown-content:visible .option", has_text=district_to_process).click()
                        wait_for_processing_to_finish(page)
                        success_dist = True
                        break
                    else:
                        print("La lista aún no contiene distritos. Reintentando...")
                        page.locator("#ten-fdist-filter-selectized").click() 
                        time.sleep(1)
                except PlaywrightTimeoutError:
                    print("El menú de Distrito no se abrió a tiempo en este intento.")
            
            if not success_dist:
                print(f"!!! FALLO CRÍTICO: No se pudo seleccionar el distrito {district_to_process} después de {max_retries_dist} intentos. Omitiendo este distrito.")
                print(f"{selected_year}-{selected_disease}-{selected_department.replace(' ', '_')}-{selected_province.replace(' ', '_')}-{district_to_process.replace(' ', '_')} HA FALLADO")
                continue # Salta a la siguiente iteración del bucle
            
            # --- Resto del proceso para el distrito seleccionado ---
            ...
            filename = (f"{selected_year}-{selected_disease}-{selected_department.replace(' ', '_')}"
                        f"-{selected_province.replace(' ', '_')}-{district_to_process.replace(' ', '_')}.xlsx")
    ...
    print("\n¡Proceso de iteración por distritos finalizado con éxito! El navegador se cerrará en 10 segundos.")
    time.sleep(10)
    browser.close()
